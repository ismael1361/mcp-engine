...
export class McpEngine<K extends string = string> {
	...
	private async cleanupIdleConnections() {
		...
		for (const [sessionId, session] of this.sessions) {
			if (now - session.lastActivity > this.IDLE_TIMEOUT_MS * 2) {
				...
				for (const [name] of this.connections.get(sessionId) || []) {
	...
	/**
	 * Limpa todas as conexões e sessões ativas, encerrando clientes e transportes associados.
	 *
	 * @example
